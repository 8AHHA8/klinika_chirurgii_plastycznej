<section class="d-flex justify-content-center">
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="pesel" name="pesel" type="text" class="mt-1 block w-full" :value="old('pesel', $user->pesel)" required autofocus autocomplete="pesel" />
            <x-input-error class="mt-2" :messages="$errors->get('pesel')" />
        </div>

        <div>
            <x-input-label for="email" :value="__('Email')" />
            <x-text-input id="e-mail" name="e-mail" type="email" class="mt-1 block w-full" :value="old('email', $user->email)" required autocomplete="username" />
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div>
                    <p class="text-sm mt-2 text-gray-800 dark:text-gray-800">
                        ...
                        <button form="send-verification" class="underline text-sm text-gray-800 dark:text-gray-800 hover:text-gray-900 dark:hover:text-gray-800 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800">
                            {{ __('Click here to re-send the verification email.') }}
                        </button>
                    </p>

                    @if (session('status') === 'verification-link-sent')
                        <p class="mt-2 font-medium text-sm text-green-600 dark:text-green-800">
